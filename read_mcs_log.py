...
filename_log = path + "plmprod-1.access.2018-08-22.log"
...
def createListFromFolder(folder_path, suffix):
    ...
    complete_data_log = createListFromFile(folder_path+file_name_array[0], suffix)
    for file_idx in range(1,len(file_name_array)):
        addListFromFileName(complete_data_log, folder_path+file_name_array[file_idx], suffix)
    return complete_data_log
...
def createListFromFile(filename,suffix):
    ...
    try: 
        with open(filename, 'rt', encoding="utf8") as f:
            if filename.endswith(suffix_log) and suffix == suffix_log:
                 array = f.readlines() 
...
def createDictFromList(data_log, tag_array = ["IP", "Time", "Method", "Request","Params","HTTP-type","Response", "?", "??", "Referer", "Useragent", "Username", "???"], split_c = "|"):
    '''
        Takes a list and splits it into the appropriate dict-parts using a tag_array with keys and a split_character
    
    Input:
        data_log: List of information
        tag_array: Keys to split into, instansiated by .log files
        split_c: character to split on, instansiated as "|" from .log files
    Output:
        dict: the full dict
    '''
    d_log = defaultdict(list)
    d_log.fromkeys(tag_array)

    for string in data_log:
        string_split = string.split(split_c)
        for tag_idx, tag in enumerate(tag_array):
            d_log[tag].append(string_split[tag_idx])
    d_log['Total'] = data_log
    return d_log

def filterUserNamesDict(d_log):
    '''
    ...
    d_log['User'] = d_log['Params']
    for row_idx, row in enumerate(d_log["User"]):
        if ("-" not in d_log["Username"][row_idx] and "/" not in d_log["Username"][row_idx] and "." not in d_log["Username"][row_idx]): #Filter away inconsistencies in the data such as useragent or HTTP-type
            d_log["User"][row_idx] = d_log["Username"][row_idx]
        else:
            if("userName=" in row and "X-Atmosphere-Transport=close&X" in row):
                d_log["User"][row_idx]  =  filterUserName(d_log["User"][row_idx], filter_string_transport)
            elif("userName=" in row and "X-Atmosphere-tracking-id=" in row):
                d_log["User"][row_idx]  =  filterUserName(d_log["User"][row_idx], filter_string_tracking_id)
    d_log["User"] = [user for user in d_log["User"] if '""' not in user and "?" not in user] 
    return d_log

def getUniqueUsers(d_log):
    '''
        Converts list to set and therefore only keep unique users, return set 
    '''
    return set(d_log["User"])
